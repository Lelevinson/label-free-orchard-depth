...
13. `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/archive/research_process_notes/SNAPSHOT_FOLDER_AUDIT_BEFORE_PRESENTATION.md` is the archived pre-presentation audit of Levinson snapshot folder tidiness, contradictions, active root-code state, and presentation readiness.
14. `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/archive/research_process_notes/LEVINSON_DOCS_OPTIMIZATION_BEFORE_PRESENTATION.md` is the archived documentation optimization audit that inventories current versus historical Levinson docs before presentation preparation.
15. `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/archive/research_process_notes/REPO_TIDY_AUDIT.md` is the archived cleanup audit for documentation/source-of-truth status, stale or overlapping notes, presentation-only artifacts, temporary/generated candidates, safe edits performed, and delete/archive actions that still need approval.
...
26. Progress-presentation assets live under `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/presentation_assets/`, which is ignored by shared `.gitignore` and should remain presentation-only rather than research source-of-truth. The Slide 4 research-evolution package at `presentation_assets/progress_presentation/slide4_comparison/` centers on the Levinson self-supervised method progression from B0/Snapshot 00 through Snapshot 07, with a two-lane experiment-evolution SVG, a pilot/gate-versus-full-run companion SVG, a backup evolution table, and concise slide materials. Slide 4 should explain failed/weak/mixed directions and the teacher-anchor turning point, not repeat Slide 3's final metric comparison. On 2026-05-25, two post-hoc matched 250-step Val100 pilots were run for Slide 4 fairness: Snapshot 05 reached median-scaled `abs_rel=0.4779`, `a1=0.4224`; Snapshot 06 reached median-scaled `abs_rel=0.5523`, `a1=0.3696`. These should be labeled as post-hoc matched gates, not as evidence that existed before the original S05/S06 full runs.
27. The repo/workspace cleanup audit is archived at `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/archive/research_process_notes/REPO_TIDY_AUDIT.md`. It classifies source-of-truth docs, historical/provenance docs, stale or overlapping docs, presentation-only assets, temporary/generated cleanup candidates, safe edits made, and delete/archive actions that require explicit approval.
...
9. For exact Snapshot 07 full-run weights and heuristic thresholds, use the snapshot `config/opt.json` and `diagnostics/full_run_last_logged.json` as source of truth; copied command scripts include later stricter sky/far and boundary arguments that do not match the archived completed-run config.
10. Validation-selected checkpoint:
...
11. CUDA smoke passed with finite losses and non-trivial new maps. Snapshot 07 pilot first-100 validation at step 250 improved the same-budget no-mask control on both median metrics: control median-scaled `abs_rel=0.5634`, `a1=0.3577`; Snapshot 07 pilot median-scaled `abs_rel=0.5497`, `a1=0.3712`.
12. Validation-only checkpoint selection evaluated `weights_0` through `weights_29` on full validation and selected `weights_25` by the B0-close `a1` rule before test evaluation.
13. Selected Snapshot 07 `weights_25` full validation result: raw `abs_rel=0.7265`, raw `a1=0.0167`, median-scaled `abs_rel=0.4344`, median-scaled `a1=0.5927`.
14. Selected Snapshot 07 `weights_25` full test result: raw `abs_rel=0.7297`, raw `a1=0.0130`, median-scaled `abs_rel=0.3840`, median-scaled `a1=0.6539`.
15. Comparison: Snapshot 07 beats B0 and Snapshot 05 `weights_19` on test median-scaled `abs_rel`, improves test median-scaled `a1` over Snapshot 05 `weights_19`, nearly matches original Lite-Mono on test median-scaled `abs_rel` (`0.3840` versus `0.3836`), and strongly beats original Lite-Mono on test median-scaled `a1` (`0.6539` versus `0.4989`). It slightly trails B0 test median-scaled `a1=0.6582`.
16. Visual judgment: promising mixed. Valid-mask panels often support the metric gains, but full-image plain inference still shows smooth vegetation masses and some sky/far-canopy weakness.
17. Conclusion: promising mixed / strongest Levinson label-free candidate so far. Treat Snapshot 07 `weights_25` as the current lead paper candidate, but do not claim the full-image visual problem is solved.
...
14. Use `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/archive/research_process_notes/REPO_TIDY_AUDIT.md` before deleting, archiving, renaming, or consolidating old Markdown/helper files; delete/archive candidates from that report still require explicit user approval.
