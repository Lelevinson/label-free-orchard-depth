...
8. The active root code remains the teacher-anchored branch shared by Snapshot 05 and Snapshot 06; use `ACTIVE_ROOT_CODE_STATE.md`, the Snapshot 06 `code/` folder, and Snapshot 05/06 patch artifacts if the shared baseline state needs to be restored later. The pre-Snapshot07 repository audit is `citrus_project/milestones/04_lightweight_vegetation_improvement/levinson/pre_snapshot07_repo_audit.md`.
9. Future Levinson follow-up should not start new training until the team reviews the Snapshot 05 `weights_19` visual packages, especially `selected_weights19_professor_visual_diagnostics/visual_diagnosis.md`, reads `ACTIVE_ROOT_CODE_STATE.md`, and decides whether the next method starts from the active teacher-anchor root code, restores baseline first, or copies a snapshot archival version intentionally.
