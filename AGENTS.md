...
11. Next direction (chosen 2026-06-03, self-supervised-pure): feature-metric loss (FeatDepth-style) first — S08 gate completed, weak/mixed, code kept off-by-default and not promoted to a snapshot; then TSOB boundary-uncertainty mixture — gate completed, a NEGATIVE result. Full 30-epoch run (laptop, batch 12) completed 2026-06-05; selected weights_24 is WORSE than Snapshot 07 on both test metrics (abs_rel 0.4331 vs 0.3840, a1 0.6492 vs 0.6539) and shows NO visible de-blobbing in rendered panels. TSOB code stays off-by-default, not promoted. Current lead remains Snapshot 07. Next direction: **Snapshot 10 designed** (DESIGN ONLY, awaiting build) — EMA in-domain self-teacher with independent DC/GC consistency gating + a scale-and-shift-aware (SI-log) metric distillation term, engineered by a 20-agent research+design+review workflow. Full spec: `snapshots/10_ema_self_teacher/DESIGN_NOTE.md`. Aimed at moving abs_rel (the SI-log term, the key fix vs S07's tie) without trading off a1; off-by-default; single-image RGB inference unchanged; expected full run ~24-28h; #1 risk is canopy keep-ratio collapse (mandatory canopy keep-ratio logging + a cheap 3-epoch pre-gate before committing). Multi-frame training-only teacher queued. Foundation-model distillation stays out of Levinson's label-free scope (Marvel/hybrid). See `LITERATURE_SHORTLIST.md` and `runs/s08_gate_logs/gate_eval_results.md`.
