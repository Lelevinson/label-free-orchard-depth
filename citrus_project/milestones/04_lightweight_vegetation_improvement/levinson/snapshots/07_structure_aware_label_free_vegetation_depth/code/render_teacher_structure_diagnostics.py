...
def load_options(run_dir: Path, weights_folder: Path, output_dir: Path, batch_size: int,
    ...
    options["model_name"] = "teacher_structure_diagnostic_loader"
...
def save_sample_maps(output_dir: Path, sample_number: int, dataset_index: int,
    ...
    map_specs = [
        (("depth", 0, 0), "student_depth_gray.png"),
        (("teacher_depth", 0), "teacher_depth_gray.png"),
        (("disp", 0), "student_disp_gray.png"),
        (("teacher_disp", 0), "teacher_disp_gray.png"),
        (("teacher_structure_student", 0), "student_normalized_structure_gray.png"),
        (("teacher_structure_teacher", 0), "teacher_normalized_structure_gray.png"),
        (("teacher_structure_diff", 0), "student_teacher_structure_diff_gray.png"),
        (("teacher_gradient_error", 0), "teacher_gradient_error_gray.png"),
        (("teacher_confidence", 0), "teacher_confidence_gray.png"),
        (("structure_rgb_edge", 0), "structure_rgb_edge_gray.png"),
        (("structure_teacher_edge", 0), "structure_teacher_edge_gray.png"),
        (("structure_boundary_reliability", 0), "structure_boundary_reliability_gray.png"),
        (("structure_sky_confidence", 0), "structure_sky_confidence_gray.png"),
        (("structure_lower_reference", 0), "structure_lower_reference_gray.png"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render teacher-anchored and structure-aware diagnostic maps.")
    parser.add_argument("--run_dir", type=Path, required=True)
    parser.add_argument("--weights_folder", type=Path, required=True)
    parser.add_argument("--output_dir", type=Path, required=True)
    parser.add_argument("--sample_indices", nargs="+", type=int, default=[0, 94, 277, 445])
...
def main() -> None:
    ...
    with (args.output_dir / "teacher_structure_diagnostic_manifest.json").open(
            "w", encoding="utf-8") as fp:
        json.dump(manifest, fp, indent=2, sort_keys=True)

    print("Saved teacher-structure diagnostics")
