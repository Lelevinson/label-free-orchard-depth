...
low-cost agricultural robots, but models tuned on urban driving data transfer
poorly to vegetation-dense scenes. Using CitrusFarm, a citrus-orchard robotics
...
exponential-moving-average (EMA) self-teacher} that distills a
scale-and-shift-invariant (SI-log) metric term plus a normalized-structure
...
%% ===========================================================================
\textbf{Self-supervised monocular depth.}
Monodepth2~\cite{godard2019monodepth2} established the modern recipe:
photometric reprojection loss across temporally adjacent frames, per-pixel
minimum reprojection, and auto-masking of static pixels. Lite-Mono~\cite{zhang2023litemono}
brings this to a $\sim$3M-parameter CNN--Transformer hybrid suitable for
embedded use; it is our backbone and baseline.

\textbf{Self-distillation and mean teachers.}
Weight-averaged ``mean teacher'' targets stabilize semi-supervised
learning~\cite{tarvainen2017meanteacher}. In depth, consistency-based
self-distillation~\cite{ecdepth2023} and augmentation-based self-distillation
(BDEdepth~\cite{bdedepth2023}, RA-Depth~\cite{he2022radepth},
AugUndo~\cite{wu2024augundo}) improve robustness without labels. Our method is in
this family: an EMA copy of the student supervises the student, but with a
scale-and-shift-invariant metric term (rooted in the scale-invariant log loss of
Eigen \emph{et al.}~\cite{eigen2014depth}) that, unlike a purely normalized
structure term, can move the scale-sensitive component of the error.

\textbf{The vertical-position shortcut.}
A line of work shows these networks infer object distance largely from vertical
image position and the ground-contact point, not apparent
size~\cite{vandijk2019howdo}; EPCDepth~\cite{peng2021epcdepth} names vertical
position the dominant cue and proposes data grafting to weaken it;
DaCCN~\cite{han2023daccn} demonstrates a strong vertical-vs-horizontal asymmetry;
and Camera Pose Matters~\cite{zhao2021camerapose} frames the bias as a
consequence of the camera-pose distribution in training data. Our diagnosis
reproduces this shortcut on vegetation and motivates (and then critiques) a
crop-equivariance attempt.

\textbf{Sharpening and multi-frame methods.}
Feature-metric losses~\cite{shu2020featdepth} and boundary
representations~\cite{tsob2025} aim to sharpen depth discontinuities; multi-frame
cost-volume methods~\cite{watson2021manydepth} achieve strong urban abs\_rel but
require multiple frames (or a cost volume) at inference unless distilled. We tried
the first two (Section~\ref{sec:negatives}); the third was rejected for
exceeding our 8\,GB training budget.

\textbf{Out of scope: supervised foundation-model distillation.}
Distilling a large supervised model such as Depth
Anything~\cite{yang2024depthanything} typically gives the largest gains but
converts the task into a supervised one. We treat this as a separate
hybrid/supervised path and exclude it from our label-free study.
...
\begin{figure}[t]
\centering
\fbox{\parbox[c][4.6cm][c]{0.90\linewidth}{\centering
\textbf{[Figure 1 --- METHOD SCHEMATIC: TO BE DRAWN BY A HUMAN]}\\[6pt]
\footnotesize Student Lite-Mono (grad) and a frozen EMA copy (no grad) both see
the clean target image. The EMA prediction supplies (a) an SI-log metric
distillation target and (b) a normalized-structure target. Two reliability gates
(GC: geometric min-reprojection error; DC: horizontal-flip consistency) mask the
loss. EMA weights update after each optimizer step. At inference the EMA copy is
discarded; only the student encoder+decoder run on a single RGB image.}}
\caption{Training-only EMA self-teacher (placeholder; a proper schematic must be
drawn). Inference is the unmodified single-image Lite-Mono depth network.}
\label{fig:method}
\end{figure}
