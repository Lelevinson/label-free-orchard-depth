...
For the main Milestone 3 result, we fine-tuned.
...
Why fine-tuning was the right Milestone 3 baseline:

1. Milestone 1 measured the untouched original Lite-Mono checkpoint on Citrus.
2. Milestone 3 asked whether normal Citrus self-supervised adaptation helps that checkpoint.
3. Milestone 4 can now ask whether a vegetation-focused method beats both the original model and normal adaptation.

Fair comparison ladder:

```text
A. original Lite-Mono, no Citrus training
B. original Lite-Mono fine-tuned on Citrus with standard self-supervision
C. improved Milestone 4 method trained with the same Citrus budget
```
...
This mattered because full training defaults can be large. A smoke run should prove that loading, logging, validation, and saving behave correctly without accidentally starting a long experiment.
...
### Does a smoke run give reliable research results?

No. A smoke run gives engineering evidence, not research evidence.

A smoke run can prove:

1. the original depth weights load
2. Citrus temporal batches flow through the trainer
3. validation does not crash
4. the safety brake stops training
5. checkpoints can be saved and loaded

A smoke run does not prove:

1. the model adapted well to Citrus
2. depth metrics improved
3. the checkpoint is useful for the paper

Plain version:

```text
smoke run = the machine starts and the controls work
research result = planned training plus evaluation against the baseline
```

### Why can photo loss improve while depth gets worse?

Self-supervised Lite-Mono training optimizes an image-matching game:

```text
Can I use predicted depth + predicted camera motion to warp a nearby RGB frame so it looks like the current RGB frame?
```

Our research evaluation asks a different question:

```text
Does the predicted depth match the LiDAR-supported Citrus depth labels where the labels are valid?
```

Those questions are related, but not identical. In vegetation scenes, repeated leaves, shadows, thin branches, occlusions, and exposure changes can let photo matching improve without producing better geometry.

Milestone 3 found exactly this risk:

```text
training photo loss can go down
while LiDAR-valid relative-depth quality gets worse
```

That is why we track both the training loss and depth metrics.
...
```text
Are the predicted meter distances close to the LiDAR-label meter distances?
```

Median-scaled metrics ask:

```text
If we correct one global scale mistake for this image, is the near/far structure still right?
```
...
Prediction A has the wrong scale, but the pattern is right: the background is much farther than the leaves.
Prediction B may look closer in some raw meter values, but the pattern is worse: the background is only slightly farther than the leaves.

Milestone 3 often showed this problem:

```text
raw distance numbers sometimes improved
relative depth structure got worse
```

For a robot, both matter eventually. For diagnosing Milestone 3, median-scaled metrics helped reveal that the model was damaging the image's internal depth layout.

### What does it mean to freeze depth while pose warms up?

In this repo, self-supervised training has two main parts:

- depth model: predicts depth/disparity from the current RGB image
- pose model: predicts camera movement between neighboring frames

The training question is:

```text
If I know the depth of the current image and the camera motion,
can I warp a nearby image so it matches the current image?
```

The risky part is that Milestone 3 starts with useful pretrained depth weights, while pose may start less adapted to Citrus. If pose is wrong early, the photo loss can push the depth model in a bad direction.
...
For the first N training steps:
  update pose
  protect the depth weights from optimizer updates
```

This is only a safety lever. It does not guarantee improvement. Milestone 3 showed that pose-only training can lower photo loss, but it does not improve the final RGB-only depth model if the depth path remains unchanged.

### What is the difference between freezing the depth encoder and updating only the decoder?
...
- encoder = the larger visual feature extractor
- decoder = the smaller part that turns features into disparity/depth

Updating only the decoder is a cautious test:
...
Milestone 3 verified that this protection worked mechanically, but the validation metrics still got worse. So the issue was not only encoder drift; the standard photo-loss adaptation signal itself was still pushing depth predictions in the wrong direction.

### What did Milestone 3 conclude?

Milestone 3 concluded that standard self-supervised Citrus adaptation is weak/negative evidence under the tested recipe family.

Important first-100 validation reference:

| setting | median-scaled abs_rel | median-scaled a1 |
|---|---:|---:|
| original Lite-Mono, no Citrus training | 0.3680 | 0.4807 |
| conservative adapted control after 1000 updates | 0.6615 | 0.1827 |
| no-color-augmentation control after 250 updates | 0.4108 | 0.4568 |
| no-color-augmentation control after 500 updates | 0.5300 | 0.3513 |
| normal batch-size-12 one-epoch control | 3.0501 | 0.2473 |

Plain meaning:

```text
Normal Citrus fine-tuning did not beat the original model.
The model learned the photo-matching objective, but its LiDAR-valid depth structure got worse.
```

This does not mean the project failed. It gives a clear target for Milestone 4:

```text
adapt to vegetation scenes while preserving or improving relative depth structure
```

### Could Milestone 3 be failing only because the runs were too short?

A longer run could always behave differently, so we should not claim that all long training can never recover.

But the controlled evidence gave a warning:

1. depth-frozen pose warmup stayed close to the original baseline
2. a few depth-update steps already started hurting relative depth
3. longer standard controls got worse, not better
4. the conservative 1000-step probe did not recover
5. the no-color-augmentation 500-step continuation worsened after the better 250-step checkpoint

Plain meaning:

```text
A blind longer run of the same recipe is not justified right now.
```

A future longer run should require a new technical reason, monitoring, and explicit confirmation.

### Could the Milestone 3 failure be caused by loading the wrong weights?

That was an advisor question, so we checked it.
...
Milestone 3 intentionally loaded:
...
- extra checkpoint tensors were profiling metadata, not model weights needed for inference
- a fully depth-frozen checkpoint stayed exactly identical to the original encoder/depth tensors

Plain conclusion:

```text
wrong depth-weight loading is probably not why Milestone 3 failed
```

### If we evaluate on training images, should the adapted model become very accurate?
...
We checked first-100 training images. Adapted checkpoints still did not become clearly better than the original model. The same pattern appeared:
...
The normal Citrus evaluation uses prepared semi-dense labels:

```text
raw LiDAR points -> projected into camera -> local_idw fill -> valid mask
```

The advisor asked whether the fill step could be causing the negative result. So we also checked only raw projected sparse LiDAR pixels:

```text
raw LiDAR points -> projected into camera -> evaluate only sparse pixels
```

That check used no `local_idw` filling. It only used about 0.96% of image pixels, so it is harsher and noisier than the normal evaluation.

First-100 validation result:
...
```text
Even when evaluating only sparse raw LiDAR points, adapted checkpoints stayed worse than the original model.
```

This does not prove the Citrus label pipeline is perfect. It only reduces the chance that Milestone 3 failed only because of `local_idw` densification.

### What does batch size mean, and did batch size 12 fix Milestone 3?

Batch size means:

```text
how many training examples the model sees before one optimizer update
```

Small example:

- batch size 4 = average loss over 4 image triplets, then update
- batch size 12 = average loss over 12 image triplets, then update

The original Lite-Mono README training example uses `--batch_size 12`. Earlier Milestone 3 controlled runs used `--batch_size 4`.

The laptop RTX 4060 GPU can fit true batch sizes 8 and 12 for one-step CUDA training, with no gradient accumulation.

But batch size 12 did not fix the training problem. In a normal one-epoch batch-size-12 control:

```text
photo loss decreased from about 0.164 to about 0.133
but median-scaled depth metrics got worse
```

Plain meaning:

```text
A larger true batch size is feasible, but it is not the main missing piece.
```

### Where should I look for the detailed Milestone 3 evidence?

Use these files instead of keeping every table inside this beginner note:

| question | best file |
|---|---|
| What is the official Milestone 3 verdict? | `citrus_project/milestones/03_self_supervised_adaptation/README.md` |
| Which run folders are evidence versus cleanup candidates? | `citrus_project/milestones/03_self_supervised_adaptation/artifact_inventory.md` |
| What should we show the professor? | `citrus_project/milestones/03_self_supervised_adaptation/professor_loading_and_train_eval_check.md` |
| What are the detailed model-behavior notes? | `citrus_project/research/baseline_notes.md` |
| What questions did the advisor ask? | `citrus_project/research/advisor_notes.md` |
| What might appear in the paper? | `citrus_project/research/paper_shortlist.md` |
